...
class FinalPacking extends MY_Controller{
    ...
    public function addPacking($jsonData=""){
		$postData = [];
		if(!empty($jsonData)):
            $postData = (Array) decodeURL($jsonData);
        endif;
		if(!empty($postData['ref_id'])){
			$this->data['chData'] = $chData = $this->deliveryChallan->getDeliveryChallan(['id'=>$postData['ref_id'],'itemList'=>1]);
			$this->data['party_id'] = $chData->party_id;
			$this->data['ref_id'] = $postData['ref_id'];
			$this->data['from_entry_type'] =  $chData->entry_type;
			
            $itemData = $this->deliveryChallan->getPackingItemDetail(['dc_id'=>$postData['ref_id']]);
            $this->data['dataRow'] = $itemData;
        }
        ...
        $this->data['packMtList'] = $this->item->getItemList(['item_type'=>3]);
        $this->data['transportList'] = $this->transport->getTransportList();
		$this->data['itemList'] = $chData->itemList;
    ...
    public function getPendingOrders($param = []){
        ...
        if(!empty($soData)){
            foreach($soData AS $row){
                if($row->is_packing != 0){
                    $options .= '<option value="'.$row->id.'" data-item_id = "'.$row->item_id.'" data-item_name="'.$row->item_name.'"  data-packing_type="'.$row->is_packing.'">'.$row->item_name .' [ SO NO : '.$row->trans_number.' | Pending Qty : '.$row->pending_qty.']'.'</option>';
                }
