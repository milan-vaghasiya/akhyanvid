...
class HsnMaster extends MY_Controller{
    ...
    // HSN Wise Update Gst
    public function updateMasterDetail(){
        $data = $this->input->post();
        $this->data['dataRow'] = $this->hsnModel->getHSNDetail(['id'=>$data['id']]);    
        $this->load->view('hsn_master/update_master_detail',$this->data);
    }

    public function saveUpdateMasterDetail(){
        $data = $this->input->post();
		$errorMessage = array();

        if(empty($data['new_gst_per']))
			$errorMessage['new_gst_per'] = "Gst is required.";

        if(!empty($errorMessage)):
            $this->printJson(['status'=>0,'message'=>$errorMessage]);
        else:
            $this->printJson($this->hsnModel->saveUpdateMasterDetail($data));
        endif;
    }
}