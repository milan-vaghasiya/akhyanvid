...
class CreditNoteModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['select'] = "id,order_type,trans_number,trans_date,party_name,taxable_amount,gst_amount,net_amount,ewb_status,eway_bill_no,party_id,e_inv_status,e_inv_no,trans_status";
    ...
    public function save($data){
        try{
            ...
            if(!empty($data['id'])):                
                ...
                $this->remove($this->stockTrans,['main_ref_id'=>$data['id'],'trans_type'=>'CRD']);
            endif;
    ...
    public function delete($id){
        try{
            ...
            $bQty = [];
            foreach($vouData->itemList as $row):
                if(!isset($bQty[$row->item_id])): $bQty[$row->item_id] = 0; endif;
                if($row->stock_eff == 1 && !empty($row->ref_id)):
                    ...
                    if($bQty[$row->item_id] > $stockQty):
                        break;
                        $this->db->trans_rollback();
                        return ['status'=>2,'message'=>"somthing is wrong. Error : Item Stock not avalible."];
                    endif;
            ...
            $this->remove($this->stockTrans,['main_ref_id'=>$id,'trans_type'=>'CRD']);
