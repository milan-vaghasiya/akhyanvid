...
class HsnMasterModel extends MasterModel{
    ...
    // HSN Wise Update Gst
    public function saveUpdateMasterDetail($data){ 
        try{
            $this->db->trans_begin();

            $result = $this->store('hsn_update_log',$data,'HSN Master');

            if(!empty($data['hsn_id'])):
                $gstData['gst_per'] = $data['new_gst_per'];
                $gstData['igst'] = $gstData['gst_per'] ;
                $gstData['cgst'] = $gstData['sgst'] = (!empty($gstData['igst']))?($gstData['igst']/2):0;

                $this->edit($this->hsnMaster,['id'=>$data['hsn_id']],$gstData);

                $this->edit('item_master',['hsn_code'=>$data['hsn']],['gst_per'=>$data['new_gst_per']]);
