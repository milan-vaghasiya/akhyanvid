...
class PartyModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        if($data['party_category'] != 4):
			...
			$data['searchCol'][] = "party_master.gstin";
    ...
    public function getParty($data){
        $queryData = array();
        $queryData['tableName']  = $this->partyMaster;
        $queryData['select'] = "party_master.*,b_countries.name as country_name,b_states.name as state_name,IF(b_states.gst_statecode > 0,b_states.gst_statecode,96) as state_code,b_cities.name as city_name,d_countries.name as delivery_country_name,d_states.name as delivery_state_name,IF(d_states.gst_statecode > 0,d_states.gst_statecode,96) as delivery_state_code,d_cities.name as delivery_city_name,IF(currency.inrrate > 0,currency.inrrate,1) as inrrate, (CASE WHEN tl.cl_balance > 0 THEN ABS(TRIM(tl.cl_balance)+0) WHEN tl.cl_balance < 0 THEN ABS(TRIM(tl.cl_balance)+0) ELSE 0 END) as closing_balance,(CASE WHEN tl.cl_balance > 0 THEN 'CR' WHEN tl.cl_balance < 0 THEN 'DR' ELSE '' END) as closing_type,tl.cl_balance";
    ...
    public function getCity($data){
        ...
        if(!empty($data['name'])):
            $queryData['where']['cities.name'] = $data['name'];
        endif;
    ...
    public function save($data){
		try {
			...
			if(!empty($data['gstin']) && $this->checkDuplicateGST($data) > 0):
	...
	public function checkDuplicateGST($data = array()){
		$queryData['tableName'] = $this->partyMaster;
        if(!empty($data['gstin']))
    ...
    public function saveOpeningBalance($data){
        try{
            ...
            if(!empty($data['id'])):
                /* $checkBillWiseRef = $this->transMainModel->checkBillWiseRef(['id'=>0,'party_id'=>$data['id'],'entry_type'=>0]);
                if($checkBillWiseRef == true):
                    return ['status'=>2,'message'=>'Bill Wise Reference already adjusted. if you want to change this account opening first unset all adjustment.'];
                endif; */
                ...
                /* if($data['opening_balance'] <> 0):
                    ...
                    $transBillWiseData = ['id'=>"",'entry_type'=>0,'trans_main_id'=>0,'trans_date'=>$this->startYearDate,'trans_number'=>"OpBal",'party_id'=>$data['id'],'amount'=>abs($data['opening_balance']),'c_or_d'=>(($data['opening_balance'] > 0)?"CR":"DR"),'ref_type'=>1];
                    $this->transMainModel->storeTransBillWise($transBillWiseData);
                endif; */
            else:
