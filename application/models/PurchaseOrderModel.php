...
class PurchaseOrderModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['select'] = "po_trans.id as po_trans_id,po_trans.trans_main_id,po_trans.qty,po_trans.item_remark,po_trans.trans_status,po_master.id,po_master.trans_number,DATE_FORMAT(po_master.trans_date,'%d-%m-%Y') as trans_date,party_master.party_name,item_master.item_name,unit_master.unit_name"; 
        ...
        $data['leftJoin']['unit_master'] = "unit_master.id = po_trans.unit_id"; 
        ...
        $data['searchCol'][] = "CONCAT(po_trans.qty, ' ',unit_master.unit_name)"; 
    ...
    public function save($data){ 
        try{
			...
			foreach($itemData as $row): 
                ...
                /*if(!empty($row['ref_id'])):
                    $this->edit('purchase_enquiry',['id'=>$row['ref_id']],['trans_status'=>4]);
                endif;*/
				if(!empty($row['req_id'])):
    ...
    public function getPurchaseOrder($data){
        $queryData = array();
        $queryData['tableName'] = $this->po_master;
        $queryData['select'] = "po_master.*, trans_details.t_col_3 as delivery_address, trans_details.t_col_4 as delivery_pincode,party_master.party_name,party_master.contact_person,party_master.party_mobile";
    ...
    public function getPurchaseOrderItems($data){
        ...
        $queryData['select'] = "po_trans.*,item_master.item_name,unit_master.unit_name,po_master.trans_number,po_master.trans_date,party_master.party_name";
        $queryData['leftJoin']['item_master'] = "item_master.id = po_trans.item_id";
        $queryData['leftJoin']['po_master'] = "po_master.id = po_trans.trans_main_id";
        $queryData['leftJoin']['party_master'] = "party_master.id = po_master.party_id";
    ...
    public function getPartyWisePoList($data){
        ...
        $queryData['leftJoin']['po_trans'] = "po_trans.trans_main_id = po_master.id";
    ...
    public function getPendingPoItems($data){
        $queryData['tableName'] = $this->po_trans;
        $queryData['select'] = "po_trans.id as po_trans_id,po_trans.item_id,item_master.item_code,item_master.item_name,po_trans.qty,po_trans.dispatch_qty as received_qty,(po_trans.qty - po_trans.dispatch_qty) as pending_qty,po_trans.price,po_trans.disc_per,item_category.location_id";
    ...
    public function approvePurchaseOrder($data) {
        try{
            ...
            $this->store($this->po_master, ['id'=> $data['id'], 'trans_status' => 3, 'is_approve' => $isApprove, 'approve_date'=>$date]);
            $this->edit($this->po_trans, ['trans_main_id'=> $data['id']], ['trans_status' => 3]);
