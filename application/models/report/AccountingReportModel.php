...
class AccountingReportModel extends MasterModel{
    ...
    public function _productOpeningAndClosingAmount($data){
        ...
        FROM (
            ...
            LEFT JOIN (	
                SELECT SUM(qty * p_or_m) AS stock_qty, ( SUM(CASE WHEN price > 0 AND p_or_m = 1 THEN (qty * price) ELSE 0 END) / SUM(CASE WHEN price > 0 AND p_or_m = 1 THEN qty ELSE 0 END) ) as avg_price, item_id FROM stock_trans WHERE is_delete = 0 AND trans_date < '$from_date' GROUP BY item_id 
            ) AS ost ON ost.item_id = pm.id 
            LEFT JOIN ( 
                SELECT SUM(qty * p_or_m) AS stock_qty, ( SUM(CASE WHEN price > 0 AND p_or_m = 1 THEN (qty * price) ELSE 0 END) / SUM(CASE WHEN price > 0 AND p_or_m = 1 THEN qty ELSE 0 END) ) as avg_price, item_id FROM stock_trans WHERE is_delete = 0 AND trans_date <= '$to_date' GROUP BY item_id 
