    ...
    if(!empty($enqData))
    {
        ...
        if($enqData->trans_status == 1){
            ...
            /*if(empty($enqData->quotation_count)){
                $quotParam = "{'postData':{'id' : ".$enqData->id."},'modal_id' : 'bs-right-md-modal', 'form_id' : 'createQuotation', 'title' : 'Create Quotation', 'call_function' : 'createQuotation', 'fnsave' : 'saveQuotation', 'js_store_fn' : 'storeEnquiry', 'res_function' : 'loadDesk'}";
                $btn .= ' <a class="dropdown-item text-primary" href="javascript:void(0)" datatip="Create Quotation" flow="down" onclick="loadform('.$quotParam.')"><i class="fa fa-file-alt"></i> Quotation</a>';
            }else{
                $approveParam = "{'postData':{'id' : ".$enqData->id."},'modal_id' : 'bs-right-md-modal', 'form_id' : 'createQuotation', 'title' : 'Approve', 'call_function' : 'approveEnquiry', 'button' : 'close'}";
                $btn .= ' <a class="dropdown-item text-primary" href="javascript:void(0)" datatip="Approve" flow="down" onclick="loadform('.$approveParam.')"><i class="fa fa-check"></i> Approve</a>';
            }*/
        }
        ...
        $enqDetail = '<div class="cd-header" style="padding: 7px 16px;">
                        ...
                        <div class="enq-body vh-35" data-simplebar>
    					    <div class="enqDetail1 ml-20">
                                ...
                                <div class="mt-2" style="border-bottom: 1px dashed #e8ebf3;">
                                    <p class="m-0 font-15">Qty<br><span class="text-muted">'.floatval($enqData->qty).' '.$enqData->unit_name.'</span></p>
                                </div>
