                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-5 form-group">
                                            ...
                                            <select name="party_id" id="party_id" class="form-control select2 partyDetails partyOptions req" data-res_function="resPartyDetail" data-party_category="1,2,3">
                                                ...
                                                <?=getPartyListOption($partyList,((!empty($dataRow->party_id))?$dataRow->party_id:(!empty($enqData->party_id)?$enqData->party_id:'')))?>
                                            </select>
                ...
                <form id="itemForm">
                    <div class="col-md-12">
                        ...
                        <div class="row form-group">
                            ...
                            <div class="col-md-6 form-group">
                                <label for="com_qty">Commercial Quantity</label>
                                <input type="text" name="com_qty" id="com_qty" class="form-control floatOnly req" value="0">
                            </div>
                            <div class="col-md-6 form-group">
                            ...
                            <div class="col-md-6 form-group">
                                <label for="unit_id">Commercial Unit</label>
                                <select name="unit_id" id="unit_id" class="form-control select2 req calcComQty">
...
<?php 
	...
	if(!empty($enqData)): 
        $enqData->row_index = "";
        $enqData->gst_per = (!empty($enqData->gst_per)) ? floatVal($enqData->gst_per) : 0;
        $enqData->disc_per = 0;
        $enqData->disc_amount = 0; 
        echo '<script>AddRow('.json_encode($enqData).');</script>';
    endif;

    if(!empty($rmList)):
        $i = 0;
        $qtyArray = array_reduce($itemsArray, function($qtyArray, $item) { $qtyArray[$item->id] = $item->qty; return $qtyArray; }, []);
        foreach($rmList as $row): 
            $row->item_id = $row->id;
			$row->com_unit = $row->uom;
			$row->com_qty = $qtyArray[$row->id];
            $row->qty =$qtyArray[$row->id];
