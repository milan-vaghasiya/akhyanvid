$(document).ready(function(){
    ...
    $(document).on('click', '.saveItem', function () {
		...
		if (formData.com_qty == "" || parseFloat(formData.com_qty) == 0) {
            $(".com_qty").html("Commercial Quantity is required.");
        }
	...
	$(document).on('change keyup','.calcComQty',function(){ 
        var qty = $('#qty').val();
		var con_val = $('#unit_id :selected').data('conversion_value');
		var com_qty = 0;
...
function AddRow(data) {
	...
	var comQtyInput = $("<input/>", { type: "hidden", name: "itemData["+countRow+"][com_qty]", class:"item_qty", value: data.com_qty });
	var comQtyErrorDiv = $("<div></div>", { class: "error com_qty" + countRow });
	cell = $(row.insertCell(-1));
	cell.html(data.com_qty);
	cell.append(comQtyInput);
	cell.append(comQtyErrorDiv);
