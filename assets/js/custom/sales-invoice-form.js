...
$(document).ready(function(){
	...
	$(document).on('click','#savePrint',function(e){
		e.stopImmediatePropagation();e.preventDefault();
		inv_print = 1;
	});
	
	$(document).on('click','.getPendingOrders',function(e){
		e.stopImmediatePropagation();e.preventDefault();
	...
	$(document).on('click','.getPendingChallan',function(e){
		e.stopImmediatePropagation();e.preventDefault();
		var party_id = $('#party_id').val();
		var party_name = $('#party_id :selected').text();
		$('.party_id').html("");
	...
	$(document).on('click','.getPendingDispatchOrder',function(e){
		e.stopImmediatePropagation();e.preventDefault();
		var party_id = $('#party_id').val();
		var party_name = $('#party_id :selected').text();
		$('.party_id').html("");
		...
		if (party_id != "" || party_id != 0) {
			$.ajax({
				url: base_url + 'dispatchOrder/getPartyOrders',
				type: 'post',
				data: { party_id: party_id, batchDetail : 1 },
				success: function (response) {
					$("#modal-xl").modal("show");
					$('#modal-xl .modal-body').html('');
    ...
    $(document).on('click', '.saveItem', function (e) {
		...
		var fd = $('#batchDetail').find('input').serializeArray();var batchDetail = [];
		$.each(fd, function (i, v) {
    ...
    $(document).on('click', '.btn-item-form-close', function (e) {
		e.stopImmediatePropagation();e.preventDefault();
		$('#itemForm')[0].reset();
		$("#itemForm input:hidden").val('')
		$('#itemForm #row_index').val("");
	...
	$(document).on('change','#hsn_code',function(e){
		e.stopImmediatePropagation();e.preventDefault();
		$("#gst_per").val(($("#hsn_code :selected").data('gst_per') || 0));
		$("#gst_per").select2();
	});

	$(document).on('change','#master_i_col_2',function(e){
		e.stopImmediatePropagation();e.preventDefault();
	...
	$(document).on('keyup change',"#itemForm #price",function(e){
		e.stopImmediatePropagation();e.preventDefault();
	...
	//on change Bill Per.
	$(document).on('keyup change','#sys_per',function(e){
		e.stopImmediatePropagation();e.preventDefault();
	...
	$(document).on('change','#trans_prefix',function(e){
		e.stopImmediatePropagation();e.preventDefault();
		
		var trans_prefix  = $("#trans_prefix").val();
		
		$.ajax({
			url:base_url + controller + "/getInvTransNo",
			type:'post',
	...
	$(document).on('keyup change','#itemForm .calculateBatchQty',function(){
		...
		console.log(parseFloat(batch_qty) +'>'+ parseFloat(stock_qty));
...
function createInvoice(){
	$("#tempItem").html('<tr id="noData"><td colspan="14" align="center">No data available in table</td></tr>');itemCount = 0;
	...
	$(".orderItem:checked").map(function() {
		...
		row.sys_price = row.org_price;
		row.stock_eff = (row.stock_eff == 0)?1:0;
		row.batch_detail = JSON.stringify(row.batch_detail);
		row.row_index = "";
		AddRow(row);
	}).get();
	...
	setTimeout(function(){
		...
		setTimeout(function(){
			if(data.stock_eff == 1){
				...
				var batchDetail = {'postData':{'item_id': data.item_id, 'party_id' : $("#party_id").val(), 'id' : data.id, 'batchDetail': data.batch_detail},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
...
function resPartyDetail(response = ""){
    ...
    if(response != ""){
		...
		if(partyDetail.delivery_address == '' || partyDetail.delivery_address == null){
			partyDetail.delivery_address = partyDetail.party_address;
		}
...
function resItemDetail(response = ""){
    if(response != ""){
		...
		if(itemDetail.item_type == 8){ 
			...
			$("#batchTransactions").addClass('hidden');
			$('#itemForm #qty').val("").prop('readonly',false);
			$("#itemForm #batchTrans").html('<tr><td colspan="4" class="text-center">No data available in table</td></tr>');
			...
			var batchDetail = {'postData':{'item_id': itemDetail.id, 'party_id' : $("#party_id").val(), 'id' : ''},'table_id':"batchDetail",'tbody_id':'batchTrans','tfoot_id':'','fnget':'getBatchWiseItemStock'};
			getTransHtml(batchDetail);
		}
    }else{
