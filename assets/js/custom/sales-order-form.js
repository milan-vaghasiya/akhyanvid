$(document).ready(function(){
	...
	$(document).on('change','#party_id',function(){
		...
		var ship_to = $('#ship_to :selected').val();
		if(ship_to == "" || ship_to == 0){
			$('#ship_to').val(party_id);
		}
	});
});
...
function resPartyDetail(response = ""){
    ...
    if(response != ""){
		...
		$("#sales_executive").val(partyDetail.sales_executive);
...
function soLastRec() {
	...
	if(valid){
		$.ajax({
			url: base_url + controller + '/getSOLastRec',
			type: 'post',
			data: { item_id:item_id },
			dataType: 'json',
			success: function (response) {
				$('#tbodySOLastRec').html("");
				$('#tbodySOLastRec').html(response.tbodyData);
				$("#soLastRec").show();
			}
		});
	}
